...
(function() {
  ...
  var APPS_URL = 'https://grabshot.dev/widget/apps.json';
  ...
  function init() {
    ...
    var title = container.getAttribute('data-title') || 'More from GrabShot';
  ...
  function render(container, apps, exclude, theme, title, cols) {
    ...
    var html = '<div style="' + containerStyle(isDark) + '">' +
      ...
      '</div>' +
      '<p style="' + poweredStyle(isDark) + '">Part of the <a href="https://grabshot.dev" style="color:' + (isDark ? '#60a5fa' : '#2563eb') + ';text-decoration:none;display:inline-block;min-height:44px;line-height:44px;" target="_blank">GrabShot</a> product suite</p>' +
      '</div>' +
      '<style>' +
